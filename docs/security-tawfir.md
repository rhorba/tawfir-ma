...
| **D**enial of Service — OTP endpoint abused to spam SMS costs or lock out users | Auth | Rate limit per phone number (5/10min) + per IP (20/10min, higher threshold to tolerate NAT/shared-IP traffic) — both DB-backed (`otp_challenges` row counts), so the check survives horizontal scaling unlike an in-memory limiter | Mitigated (2026-08-01) — fixed-window counting only; exponential backoff is a smaller follow-up enhancement, not yet built |
